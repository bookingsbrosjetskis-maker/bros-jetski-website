...
4. **Email** — create a Resend account and set `RESEND_API_KEY` plus `ADMIN_NOTIFY_EMAIL` (any working mailbox). Leave `EMAIL_FROM` **empty until the business domain is bought**: mail then sends from Resend's sandbox sender, which needs no DNS but only delivers to the Resend account owner. Once the domain is registered, add it under Resend > Domains, add the DNS records, wait for verification, then set `EMAIL_FROM="Bros Jetskis Rental <bookings@yourdomain.ca>"` and redeploy — that one variable is the whole switch from test to live email. **Admin > Dashboard has an "Email notifications" panel** showing which of the three settings are still missing. Without `RESEND_API_KEY`, all mail is only logged to the server console.

   Emails currently sent: booking confirmation, safety-checklist link, and booking cancellation (to the customer); new booking, new enquiry, new review, and submitted checklist (to the owner); plus an acknowledgement to anyone who uses the contact, corporate, or gift-card forms. Cancellations by the business say the deposit is *not* forfeited; customer cancellations say it is refunded.
